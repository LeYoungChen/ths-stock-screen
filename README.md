...
硬性要求分析全A股市场，不能降级为旧候选池。WorkBuddy必须直接调用同花顺ifind-mcp连接器，分区/分页取得完整名录，再逐批取指标；正式生成入口会拒绝不完整范围。

运行时先展示已核实的全A股数量，用户确认后开始；沿用原策略，不逐项询问技术参数。缺项会标待核验，范围发生变化才重新确认。
...
- 股票名录不完整时停止正式交付；指标缺失保留全市场记录并标待核验，不下“全市场无股符合”的结论。
